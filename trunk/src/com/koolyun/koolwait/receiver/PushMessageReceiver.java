package com.koolyun.koolwait.receiver;
...
import com.koolyun.koolwait.model.SharePreferenceUtil;
...
public class PushMessageReceiver extends FrontiaPushMessageReceiver {
	...
	public static void setMainActivity(MainActivity o) {
		mainActivity = o;
	}
	...
	public void onBind(Context context, int errorCode, String appid, String userId, String channelId, String requestId) {
		...
		if (errorCode == 0) {
			...
			spu.setSettingsBaiduPushBind(true, userId);
		}
	...
	public void onMessage(Context context, String message, String customContentString) {
		...
		for (RunningTaskInfo taskInfo : taskInfoList) {
			if (taskInfo.baseActivity.equals(intent.resolveActivity(context.getPackageManager()))) { // 说明它已经启动了
				if (!MainActivity.active && !ManageActivity.isVisible) {
					intent.putExtra("msg", message);
					intent.setAction(PushUtils.ACTION_MESSAGE);
					intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
					context.getApplicationContext().startActivity(intent);
				} else {
					mainActivity.newPushMessage(message);
				}
				break;
			}
		}
	...
	public void onUnbind(Context context, int errorCode, String requestId) {
		...
		if (errorCode == 0) {
			SharePreferenceUtil spu = new SharePreferenceUtil(context, SharePreferenceUtil.FILE_SETTINGS);
			spu.setSettingsBaiduPushBind(false, "");
		}
